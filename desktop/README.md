...
兩個欄位填哪個就跑哪個,都填就依序都跑。檔案一律先存到你的「下載」資料夾。

**Modules 下載完成後會自動上傳到 Supabase**(完整 BOM 不會,只當輔助參考,一樣
要去「上傳 BOM」手動匯入)。上傳前會用 `fid_machine_map` 表把 FID 對應到機台
名稱:查得到就直接上傳;查不到(或這次只填了 SO 沒填 FID)就會在畫面上多跳出
一個「機台名稱」欄位請你輸入一次,輸入後會記住這個對應,下次同一個 FID 就不用
再問。合併後 xlsx 裡每個分頁(模組)會各自變成該機台底下的一個子項,上傳完會
自動跑一次既有的「重要零件自動比對」。
