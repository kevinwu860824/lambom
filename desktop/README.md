...
- FID→SO 反查(`resolve_so_from_fid`)如果同一個 FID 的 BOM 改版過,VA03 可能
  ...
  往下位移 129)是從真實案例反推出來的,1 筆跟 2 筆的情境都驗證過;3 筆以上
  是同樣間距外推,還沒實際驗證。清單「欄」的位置每次搜尋不一定相同(不同
  FID 觀察到欄 3、4、7 都出現過),所以改成程式先掃過一段欄位範圍找出這次
  用的是哪一欄,不是猜固定值。遇到選錯時要核對 log 印出的筆數/欄位/選到的
  SO 是否正確。
